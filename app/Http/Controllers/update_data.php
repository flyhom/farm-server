...
class update_data extends BaseController
{
    ...
    public function update(Request $request)
    {
        ...
        foreach ($rows as $row)
        {
            ...
            if ($type != 'rainfall') {
               ...
               for ($i=0; $i < count($record); $i++) {
                   if ($i != 0) {
                       $raindatetime = (string)($rain_y_m[0]) . ':' . (string)($rain_y_m[1]) . ':' . (string)((int)($record[0])) . ' ' . (string)($i-1) . ':00:00';
        ...
        return response()->json(['status' => 200, 'msg' => "成功", 'sensor' => $type, 'max' => count($arr) , 'end' => $i - 1 , 'duration' => $duration]);
    }
}
